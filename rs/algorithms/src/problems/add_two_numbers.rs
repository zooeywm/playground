...
impl ListNode {
	#[inline]
	fn new(value: i32) -> Self { Self { value, next: None } }
}
...
fn _add_two_numbers(mut list1: Option<Box<ListNode>>, mut list2: Option<Box<ListNode>>) -> Vec<i32> {
	let mut dummy_head = ListNode::new(0);
	let mut current = &mut dummy_head;
	let mut carry = 0;
...
fn test() {
	...
	assert_eq!(
		_add_two_numbers(Some(Box::new(ListNode::from(vec![0]))), Some(Box::new(ListNode::from(vec![0])))),
		vec![0]
	);
